...
def gen_a16w16_nosplit_gfx942_instance(
    ...
    **_unused,
    ...
    t_k = (waves_per_wg // (k.T_M * k.T_N)) if is_wkc else 1
...
def _validate_a16w16_wave_k_coop_gfx942(k: OpusGemmInstance):
    ...
    if k.T_M < 1 or k.T_N < 1:
        errors.append(f"T_M/T_N=({k.T_M},{k.T_N}) must be positive")
    elif waves_per_wg % (k.T_M * k.T_N) != 0:
        errors.append(
            f"T_M*T_N={k.T_M * k.T_N} must divide waves_per_wg={waves_per_wg}"
        )
    if (k.W_M, k.W_N, k.W_K) != (16, 16, 16):
        errors.append(f"WAVE=({k.W_M},{k.W_N},{k.W_K}) must be (16,16,16)")
    ...
    t_k = waves_per_wg // (k.T_M * k.T_N) if k.T_M and k.T_N else 0
    a_pad = 4
