...
struct wkc_smem {
    ...
    static constexpr int A_SLABS = T::T_M * T::T_K;
    static constexpr int A_BYTES = A_WAVE_BYTES * A_SLABS * STAGES;
    // A is shared across N wave groups; B is shared across M wave groups.
    ...
    static constexpr int B_SLABS = T::T_N * T::T_K;
    static constexpr int B_BYTES = B_WAVE_BYTES * B_SLABS * STAGES;
...
OPUS_D inline void store_a_wave_to_lds(char* smem_a_base, int wave_slab_id,
                                       int stage, const V& va, int lane_id)
{
    constexpr int VEC = T::VEC_A;
    constexpr int LANES = opus::get_warp_size();
    constexpr int THREADS_K = T::B_K / VEC;
    ...
    typename T::D_A* s = reinterpret_cast<typename T::D_A*>(
        smem_a_base + (stage * wkc_smem<T>::A_SLABS + wave_slab_id) *
        wkc_smem<T>::A_WAVE_BYTES);
    int m_in_lane = lane_id / THREADS_K;
    int k_off     = (lane_id % THREADS_K) * VEC;
    #pragma unroll
    for (int i = 0; i < N_LOADS; ++i) {
...
OPUS_D inline void store_b_wave_to_lds(char* smem_b_base, int wave_slab_id,
                                       int stage, const V& vb, int lane_id)
{
    constexpr int VEC = T::VEC_B;
    constexpr int LANES = opus::get_warp_size();
    constexpr int THREADS_K = T::B_K / VEC;
    ...
    typename T::D_B* s = reinterpret_cast<typename T::D_B*>(
        smem_b_base + (stage * wkc_smem<T>::B_SLABS + wave_slab_id) *
        wkc_smem<T>::B_WAVE_BYTES);
    int n_in_lane = lane_id / THREADS_K;
    int k_off     = (lane_id % THREADS_K) * VEC;
    #pragma unroll
    for (int i = 0; i < N_LOADS; ++i) {
                                            ...
                                            float4_acc* acc)
{
    ...
    if constexpr (E_M == 1 && E_N == 1 && E_K == 4) {
        short4_ab a0 = read_a_pair_from_lds<T>(smem_a_base, a_slab_id, stage, 0, 0, lane_id);
        short4_ab b0 = read_b_pair_from_lds<T>(smem_b_base, b_slab_id, stage, 0, 0, lane_id);
        short4_ab a1 = read_a_pair_from_lds<T>(smem_a_base, a_slab_id, stage, 0, 1, lane_id);
        short4_ab b1 = read_b_pair_from_lds<T>(smem_b_base, b_slab_id, stage, 0, 1, lane_id);
        short4_ab a2 = read_a_pair_from_lds<T>(smem_a_base, a_slab_id, stage, 0, 2, lane_id);
        short4_ab b2 = read_b_pair_from_lds<T>(smem_b_base, b_slab_id, stage, 0, 2, lane_id);
        short4_ab a3 = read_a_pair_from_lds<T>(smem_a_base, a_slab_id, stage, 0, 3, lane_id);
        short4_ab b3 = read_b_pair_from_lds<T>(smem_b_base, b_slab_id, stage, 0, 3, lane_id);
        ...
        short4_ab a00 = read_a_pair_from_lds<T>(smem_a_base, a_slab_id, stage, 0, 0, lane_id);
        short4_ab a10 = read_a_pair_from_lds<T>(smem_a_base, a_slab_id, stage, 1, 0, lane_id);
        short4_ab b0  = read_b_pair_from_lds<T>(smem_b_base, b_slab_id, stage, 0, 0, lane_id);
        short4_ab a01 = read_a_pair_from_lds<T>(smem_a_base, a_slab_id, stage, 0, 1, lane_id);
        short4_ab a11 = read_a_pair_from_lds<T>(smem_a_base, a_slab_id, stage, 1, 1, lane_id);
        short4_ab b1  = read_b_pair_from_lds<T>(smem_b_base, b_slab_id, stage, 0, 1, lane_id);
        short4_ab a02 = read_a_pair_from_lds<T>(smem_a_base, a_slab_id, stage, 0, 2, lane_id);
        short4_ab a12 = read_a_pair_from_lds<T>(smem_a_base, a_slab_id, stage, 1, 2, lane_id);
        short4_ab b2  = read_b_pair_from_lds<T>(smem_b_base, b_slab_id, stage, 0, 2, lane_id);
        short4_ab a03 = read_a_pair_from_lds<T>(smem_a_base, a_slab_id, stage, 0, 3, lane_id);
        short4_ab a13 = read_a_pair_from_lds<T>(smem_a_base, a_slab_id, stage, 1, 3, lane_id);
        short4_ab b3  = read_b_pair_from_lds<T>(smem_b_base, b_slab_id, stage, 0, 3, lane_id);
        ...
        short4_ab a0 = read_a_pair_from_lds<T>(
            smem_a_base, a_slab_id, stage, 0, 0, lane_id);
        short4_ab b0 = read_b_pair_from_lds<T>(
            smem_b_base, b_slab_id, stage, 0, 0, lane_id);
        short4_ab a1 = read_a_pair_from_lds<T>(
            smem_a_base, a_slab_id, stage, 1, 0, lane_id);
        short4_ab b1 = read_b_pair_from_lds<T>(
            smem_b_base, b_slab_id, stage, 1, 0, lane_id);
...
#define OPUS_WKC_E2N2_STEP(K_NEXT)                                                   \
        ...
        {                                                                             \
            short4_ab na0 = read_a_pair_from_lds<T>(                                  \
                smem_a_base, a_slab_id, stage, 0, K_NEXT, lane_id);                   \
            short4_ab nb0 = read_b_pair_from_lds<T>(                                  \
                smem_b_base, b_slab_id, stage, 0, K_NEXT, lane_id);                   \
            short4_ab na1 = read_a_pair_from_lds<T>(                                  \
                smem_a_base, a_slab_id, stage, 1, K_NEXT, lane_id);                   \
            short4_ab nb1 = read_b_pair_from_lds<T>(                                  \
                smem_b_base, b_slab_id, stage, 1, K_NEXT, lane_id);                   \
...
#undef OPUS_WKC_E2N2_STEP
        ...
        for (int e_k = 0; e_k < E_K; ++e_k) {
            ...
            for (int e_m = 0; e_m < E_M; ++e_m)
                a_buf[e_m] = read_a_pair_from_lds<T>(smem_a_base, a_slab_id, stage, e_m, e_k, lane_id);
            #pragma unroll
            for (int e_n = 0; e_n < E_N; ++e_n)
                b_buf[e_n] = read_b_pair_from_lds<T>(smem_b_base, b_slab_id, stage, e_n, e_k, lane_id);
            #pragma unroll
            for (int e_m = 0; e_m < E_M; ++e_m) {
                #pragma unroll
                for (int e_n = 0; e_n < E_N; ++e_n) {
                                                  ...
                                                  const float4_acc* acc, int lane_id)
{
    ...
    for (int e_m = 0; e_m < E_M; ++e_m) {
        #pragma unroll
        for (int e_n = 0; e_n < E_N; ++e_n) {
            int m_base = wave_id_m * wkc_layout<T>::TILE_M + e_m * T::W_M + m_sub * 4;
            int n_pos  = wave_id_n * wkc_layout<T>::TILE_N + e_n * T::W_N + n;
...
OPUS_D inline void store_wave_acc_to_y(Y_T* ptr_y, int row, int col,
                                        int m_total, int n_total,
                                        int stride_y, const float4_acc* acc,
                                        int wave_id_m, int wave_id_n,
                                        int lane_id)
{
    constexpr int E_M = wkc_layout<T>::E_M;
    ...
    for (int e_m = 0; e_m < E_M; ++e_m) {
        ...
        for (int e_n = 0; e_n < E_N; ++e_n) {
            ...
            for (int i = 0; i < 4; ++i) {
                int gm = row + m_base + i;
                int gn = col + n_pos;
                if (gm < m_total && gn < n_total) {
                    ptr_y[gm * stride_y + gn] = static_cast<Y_T>(a[i]);
                }
            }
        }
    }
...
void gemm_a16w16_wave_k_coop_kernel(opus_gemm_noscale_kargs kargs)
{
    ...
    const int wave_id_k = wave_id % T::T_K;
    const int wave_id_tmp = wave_id / T::T_K;
    const int wave_id_n = wave_id_tmp % T::T_N;
    const int wave_id_m = wave_id_tmp / T::T_N;
    ...
    const int wave_m_offset = wave_id_m * wkc_layout<T>::TILE_M;
    const int wave_n_offset = wave_id_n * wkc_layout<T>::TILE_N;
    const int wave_k_offset = wave_id_k * T::B_K;
    const int a_slab_id = wave_id_m * T::T_K + wave_id_k;
    const int b_slab_id = wave_id_n * T::T_K + wave_id_k;
    ...
    for (int t = 0; t < wg_k_loops; ++t) {
        int stage = 0;
        store_b_wave_to_lds<T>(smem_b, b_slab_id, stage, vb, lane_id);
        if constexpr (T::T_N == 1) {
            store_a_wave_to_lds<T>(smem_a, a_slab_id, stage, va, lane_id);
        } else if (wave_id_n == 0) {
            store_a_wave_to_lds<T>(smem_a, a_slab_id, stage, va, lane_id);
        }
        if constexpr (T::T_M > 1 || T::T_N > 1) {
            s_waitcnt_lgkmcnt(0_I);
        ...
        wave_compute_one_k_tile<T>(
            smem_a, smem_b, a_slab_id, b_slab_id, stage, lane_id,
            acc);
        if constexpr (T::T_M > 1 || T::T_N > 1) {
            __builtin_amdgcn_s_barrier();
        }
    ...
    if constexpr (T::T_K == 1) {
        store_wave_acc_to_y<T, D_C>(
            ptr_y, row, col, kargs.m, kargs.n, kargs.stride_c,
            acc, wave_id_m, wave_id_n, lane_id);
        return;
    }
    ...
    store_wave_acc_to_lds_partial<T>(
        smem_partial, wave_id_k, wave_id_m, wave_id_n, acc, lane_id);
    s_waitcnt_lgkmcnt(0_I);
    __builtin_amdgcn_s_barrier();
